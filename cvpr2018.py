...
def get_pdf(data):
    href, title = data
    name = re.sub(r'[\\/:*?"<>|]', ' ', title) 
    if os.path.isfile("cvpr2018/%s" % name):
        print("File already exsists, skip %s" % name)
        return
    try:
        content = requests.get(prefix+href).content
        with open(save_dir+"/cvpr18-%s.pdf" % name, 'wb') as f:  # You may change to "path/to/your/folder"
            f.write(content)
...
if not os.path.exists(save_dir):
    os.mkdir(save_dir)
html = requests.get(prefix+'/CVPR2018.py').content
